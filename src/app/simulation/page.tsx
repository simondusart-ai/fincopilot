...
import { Badge, ErrorBox, Loading, Page, btnSecondary, inputBase, usePortalData } from '@/components/shell';
...
// Champs d'hypotheses editables dans le panneau (recalcul instantane, sans ecriture en base).
...
export default function SimulationPage() {
  ...
  const [pnl, setPnl] = useState<PnlYearRow[] | null>(null);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    Promise.all([loadSimulationAssumptions(), loadActuals()])
      .then(([r, a]) => {
        if (cancelled) return;
        setRow(r);
        setPnl(a.pnlYears);
  ...
  const realized = useMemo(() => {
    if (!pnl || !assumptions || pnl.length === 0) return null;
    const sorted = [...pnl].sort((x, y) => x.year - y.year);
  ...
  if (!isLeader) {
    return (
      <Page data={data}>
        <h1 className="text-2xl font-bold text-ink">Simulation pluriannuelle</h1>
        <p className="mt-4 rounded-2xl border border-lav bg-white p-5 text-sm text-ink/70">
          Cet écran est réservé à la direction (CFO, CEO).
        </p>
      </Page>
    );
  }

  const lastYear = realized ? realized.pnlK[realized.pnlK.length - 1].year : data.company.budget_year - 1;
  ...
  return (
    <Page data={data}>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-ink/60">
            Deux trajectoires bâties sur les mêmes hypothèses : « as is » (S&amp;M au rythme historique) et
            « rebound » (S&amp;M gelé). Projection annuelle sur trois exercices.
          </p>
        </div>
        {/* Bascule de scenario */}
      ...
      {loadErr && <div className="mt-4"><ErrorBox message={loadErr} /></div>}

      {row === undefined ? (
        <Loading />
      ) : row === null ? (
        // Aucune hypothese seedee (ex. Hexafloor) : etat vide explicatif.
        <div className="mt-6 rounded-2xl border border-lav bg-white p-6 text-sm text-ink/70">
          <p className="font-semibold text-ink">Aucune hypothèse de simulation n’est renseignée pour cette société.</p>
          <p className="mt-2">
            La simulation pluriannuelle a besoin d’un jeu d’hypothèses (croissance, marge brute, trajectoire du S&amp;M,
      ...
      ) : !assumptions || !realized || !result || !form ? (
        <Loading />
      ) : (
        (() => {
            ...
            <>
              ...
              <section className="mt-6 rounded-2xl bg-white p-5 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <h2 className="font-semibold text-ink">Hypothèses</h2>
                  <button onClick={() => row && setForm(formFromRow(row))} className={btnSecondary}>Réinitialiser</button>
                </div>
              ...
              <section className="mt-8 overflow-hidden rounded-2xl bg-white shadow-sm">
                <h2 className="px-5 pt-5 font-semibold text-ink">P&amp;L annuel projeté (K€)</h2>
              ...
              <section className="mt-8 overflow-hidden rounded-2xl bg-white shadow-sm">
                <div className="flex flex-wrap items-center gap-3 px-5 pt-5">
                  <h2 className="font-semibold text-ink">Trésorerie projetée (K€)</h2>
                ...
                <p className="px-5 pb-4 pt-2 text-xs text-ink/50">
                  Convention : flux opérationnel ≈ EBITDA (BFR neutre, capex non significatif). Vue annuelle.
                </p>
              </section>
              ...
              {mode === 'rebound' && (
                <section className="mt-8 rounded-2xl bg-white p-5 shadow-sm">
                  <div className="flex items-center gap-2">
                    <h2 className="font-semibold text-ink">L’effort traduit en CAC</h2>
                  ...
                  <p className="mt-1 text-xs text-ink/50">
                    À S&amp;M gelé, tenir la croissance de N+1 revient à acquérir un volume de clients pour un CAC équivalent donné.
                  </p>
                  ...
                  {row.cac_trajectory && row.cac_trajectory.length > 0 && (
                    <p className="mt-4 text-xs text-ink/60">
                      Trajectoire trimestrielle de CAC visée (rappel informatif) :{' '}
              ...
              <section className="mt-8 rounded-2xl bg-white p-5 shadow-sm">
                <h2 className="font-semibold text-ink">Trésorerie de clôture : as is vs rebound</h2>
