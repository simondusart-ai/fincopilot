...
- **Le moteur** (`src/lib/engine/`) : module TypeScript pur, sans dépendance à la base ni au front. Entrées : configuration société + navettes. Sorties : P&L mensuel, contributions, KPIs, alertes. Le module `actuals.ts` calcule en plus les indicateurs mensuels réalisés (base clients, churn logo, ARPA implicite, NRR, CAC, burn, runway) pour l'écran de pilotage ; `business-case.ts` chiffre un projet d'investissement (cash-flows, VAN, payback). Déterministe, couvert par la suite de tests (`npm test`, 59 tests dont les calculs vérifiés à la main). Aucun texte généré par IA : toutes les alertes sont des constats calculés.
...
- **Le portail** (Next.js) : login, saisie des navettes avec versions, dashboard de consolidation, comparateur de versions, écran de pilotage (indicateurs mensuels réalisés et pont vers le budget), générateur de business case (VAN, payback, pré-remplissage de navette), réglages du cadrage, export Excel du classeur codir.
...
1. `npm install`
2. Exécuter les migrations dans l'ordre dans l'éditeur SQL Supabase : `0001_init.sql`, `0002_new_driver_kinds.sql`, puis `0003_actuals.sql` (historique réalisé et cibles de pilotage) et `0004_business_cases.sql`.
