...
- **Le moteur** (`src/lib/engine/`) : module TypeScript pur, sans dépendance à la base ni au front. Entrées : configuration société + navettes. Sorties : P&L mensuel, contributions, KPIs, alertes. Le module `actuals.ts` calcule en plus les indicateurs mensuels réalisés (base clients, churn logo, ARPA implicite, NRR, CAC, burn, runway) pour l'écran de pilotage ; `business-case.ts` chiffre un projet d'investissement (cash-flows, VAN, payback) et injecte les cas acceptés dans la consolidation. Déterministe, couvert par la suite de tests (`npm test`, 62 tests dont les calculs vérifiés à la main). Aucun texte généré par IA : toutes les alertes sont des constats calculés.
- **Les données** (Supabase / Postgres) : toute la configuration société vit en tables (départements, drivers, enveloppes, plafonds CAC, seuils de runway, conventions), jamais dans le code. La sécurité par ligne (RLS) limite chaque Head of à son département ; le CFO voit tout et administre le référentiel ; le CEO voit tout en lecture et arbitre les business cases, sans éditer le cadrage. Une navette soumise est figée par les policies mêmes de la base.
- **Le portail** (Next.js) : login, saisie des navettes avec versions, dashboard de consolidation, comparateur de versions, écran de pilotage (indicateurs mensuels réalisés et pont vers le budget), générateur de business case (VAN, payback, arbitrage impactant la navette et la consolidation), réglages du cadrage, export Excel du classeur codir.
...
1. `npm install`
2. Exécuter les migrations dans l'ordre dans l'éditeur SQL Supabase : `0001_init.sql`, `0002_new_driver_kinds.sql`, puis `0003_actuals.sql` (historique réalisé et cibles de pilotage) , `0004_business_cases.sql` et `0005_business_case_arbitration.sql` (arbitrage et rôle CEO).
